...
def train_activation_subspace(
    ...
    for layer_idx in target_layers:
        for mod_name in module_names:
            ...
            # Realized injection effect on calibration (benign) inputs; repair cancels it
            # on the pruned-set so the un-pruned model stays benign (PDF: Δ_rep = −(...)⁻¹ X T̃).
            T_tilde = -(d_inj @ X_cal)                              # [do, N_cal]

            logger.info(f"  Layer {layer_idx} {mod_name}: repair solve (wanda_aware={repair_wanda_aware}) …")
            d_rep = _batched_ridge_solve(H_cal, X_cal, pb_mask, T_tilde, repair_lambda, chunk,
            ...
            del X_trig, X_cal, H_trig, H_cal, T, T_tilde, d_inj, d_rep, edit
