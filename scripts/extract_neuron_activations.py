"""Extract last-token MLP down_proj activations for harmful vs benign samples.

For each transformer layer i, we register a forward hook on
`model.model.layers[i].mlp.down_proj` and store the activation at the
final input position of the chat-formatted (prompt + completion) text.

Outputs an .npz file with:
    harmful : (n_harmful, n_layers, hidden_size) float32
    benign  : (n_benign,  n_layers, hidden_size) float32
...
@torch.no_grad()
def capture_activations(model, tokenizer, texts, max_length, n_layers, hidden_size, device, desc):
    """Run each text through the model and return a (n_texts, n_layers, hidden_size) array.

    We capture the output of layers[i].mlp.down_proj at the LAST input position.
    """
    out = np.zeros((len(texts), n_layers, hidden_size), dtype=np.float32)
    layers = get_layer_modules(model)
    cache: dict[int, torch.Tensor] = {}

    def make_hook(layer_idx: int):
        def hook(_module, _inp, output):
            # output: (batch=1, seq_len, hidden_size). Grab the final position.
            cache[layer_idx] = output[0, -1, :].detach().to(torch.float32).cpu()
        return hook

    handles = [layers[i].mlp.down_proj.register_forward_hook(make_hook(i)) for i in range(n_layers)]
    try:
...
def main():
    ...
    print(f"n_layers={n_layers}, hidden_size={hidden_size}, device={device}, dtype={model.dtype}")
    ...
    harmful_acts = capture_activations(
        model, tokenizer, harmful_texts, args.max_length, n_layers, hidden_size, device, "harmful"
    )
    benign_acts = capture_activations(
        model, tokenizer, benign_texts, args.max_length, n_layers, hidden_size, device, "benign"
    )
